...
class CartPoleGame():
    ...
    def initialize(self) -> None:
        ...
        state_encodings = self.generate_all_encoded_states(self.n_tilings, self.feat_bins)
        ...
        self.state_to_encoding = {self.states[i]: state_encodings[i] for i in range(len(self.states))}
        ...
        self.current_state_parameters = [self.x[0], self.dx[0], self.theta[0], self.dtheta[0], self.elapsed_time]
        current_encoded_state = get_tile_coding(self.current_state_parameters, self.tilings)
        self.current_state = self.encoding_to_state[str(current_encoded_state)]

    def get_next_state(self, action: int) -> Tuple[int, int]:
        ...
        """
        action = 10 - 20 * action
        elapsed_time = self.state_parameters[-1]
        ...
        self.state_parameters = next_state_parameters
        encoded_next_state = get_tile_coding(next_state_parameters, self.tilings)
        next_state = self.encoding_to_state[str(encoded_next_state)]
        reward = 1000 if self.is_winning_state(next_state) else -1000 if self.is_losing_state(next_state) else 1
...
if __name__ == "__main__":
    ...
    test.initialize()
    test_state = 50
    encoded_state = test.state_to_encoding[test_state]
    print(encoded_state)
