...
def gennet2(num_classes=10,lrate=0.01,opt='SGD',loss='categorical_crossentropy',act='relu',
    ...
    x = input
    if conv:  # Starting off with a convolution layer followed by max pooling
        x = KER.layers.Conv1D(16, kernel_size=(5))(x)  # Each new layer creator takes upstream layer as input
    ...
    model = KER.models.Model(input,output)  # KERAS now knows how to connect input to output
    model.compile(optimizer=opt(lr=lrate),loss=loss,metrics=[KER.metrics.categorical_accuracy])
    return model
...
def leco(epochs=100,ncases=500,seg0=0,seg1=8, vlen=50,vf=0.2,lrate=0.1,act='relu',
    ...
    cases = TFT.gen_segmented_vector_cases(vlen,ncases,minsegs=seg0,maxsegs=seg1,poptargs=True)
    in_shape = (vlen, 1) if conv else (vlen)  # 1 = no. input channels
    inputs = np.array([np.array(c[0]).reshape(in_shape) for c in cases]).astype(np.float32)
    targets = np.array([c[1] for c in cases],dtype='float')
    # ** Build the neural net, and create a tensorboard callback
    nn = gennet(num_classes = tlen,lrate=lrate,act=act,conv=conv,loss=loss)  # Create the neural net (a.k.a. "model")
    ...
    KDTFC.fireup_tensorboard(logdir)  # To view, open browser to URL = localhost:6006
    return nn, logdir
...
def leco2(epochs=100,im0=0,im1=8,ncases=500, dims=(50,50),gap=1,vf=0.2,lrate=0.01,mbs=16,act='relu'):
    ...
    nn.fit(inputs,targets,epochs=epochs,batch_size=mbs,validation_split = vf,verbose=2,callbacks=[tb_callback])
...
def leco3(epochs=100,ncases=500,seg0=0,seg1=8, vlen=50,vf=0.2,lrate=0.001,act='relu',conv=True,mbs=16,verb=1,tb=True):
    ...
    nn = gennet2(num_classes = tlen,lrate=lrate,act=act,conv=conv,in_shape=in_shape)
    ...
    sgd.fit(inputs,targets,epochs=epochs,mbs=mbs,vfrac=vf, verbosity=verb, callbacks=cbs)
    if tb: KDTFC.fireup_tensorboard(logdir)  # Open a browser to URL = localhost:6006
    return nn, logdir
...
def testelig(epochs=5,ncases=10, vlen=5,lrate=0.005,act='relu',lastact='sigmoid',opt='SGD'):
    ...
    nn = KMOD.Model(in_layer,out_layer, name='elig_test_net') # Create NN = "model"
    optimizer = eval('KOPT.'+ opt)
    nn.compile(loss='MSE', optimizer=optimizer(learning_rate=lrate))
    nn.summary()
    tdelg = KDTFC.TDCriticEligTracer(nn)
    for _ in range(epochs):
        inputs = np.random.uniform(0, 1, size=(ncases, vlen)).astype(np.float32)
...
def gen_tensorboard_callback(model, basedir="tblog/fit/",freq=1,upfreq='epoch'):
    ...
    cb = KCALL.TensorBoard(log_dir=ldir, histogram_freq=freq, update_freq=upfreq,write_graph=True)
    cb.set_model(model)
    return cb, ldir
